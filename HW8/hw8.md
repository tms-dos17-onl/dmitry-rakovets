...
```
dmitry@Mint-Study:~$ rsync -avz /tmp/alice centos@172.25.4.2:/tmp/alice
``` 
## 16. Удалить пользователей alice и bob вместе с домашними директориями.
```
dmitry@Mint-Study:~$ sudo deluser --remove-home alice
[sudo] пароль для dmitry:      
Идёт поиск файлов для сохранения/удаления ...
Удаляются файлы ...
Удаляется пользователь «alice» ...
Предупреждение: в группе `alice' больше никого нет.
Готово.
dmitry@Mint-Study:~$ sudo deluser --remove-home bob
Идёт поиск файлов для сохранения/удаления ...
Удаляются файлы ...
Удаляется пользователь «bob» ...
Предупреждение: в группе `bob' больше никого нет.
Готово.

``` 
## 17. С помощью утилиты htop определить какой процесс потребляет больше всего ресурсов в системе.

![task17] (/home/dmitry/TechMeSkills/dmitry-rakovets/HW8/htop.png)
